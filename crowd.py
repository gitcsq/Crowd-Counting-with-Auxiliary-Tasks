...
class Base(data.Dataset):
    ...
    def train_transform(self, img, keypoints):
        ...
        gt_densityMap = cv2.resize(gt_densityMap, (down_w, down_h),
                                   interpolation=cv2.INTER_CUBIC) * self.d_ratio * self.d_ratio
...
class Crowd_sh(Base):
    ...
    def train_transform(self, img, keypoints):
        ...
        return self.trans(img), torch.tensor(len(keypoints)).float(), torch.from_numpy(
            gt_densityMap.copy()).float()
...
if __name__ == '__main__':
    ...
    def train_collate(batch):
        ...
        gt_densityMaps = torch.stack(transposed_batch[2], 0)
        return images, gt_count, gt_densityMaps
    ...
    dataloaders = {x: DataLoader(datasets[x],
                        ...
                        for x in ['train', 'val']}
    for step, (inputs, gt_count, gt_densityMap) in enumerate(dataloaders['train']):
        print(inputs.size(), gt_count.size(), gt_densityMap.size())
        print(gt_count.numpy(), torch.sum(gt_densityMap.squeeze(0).squeeze(0)))
        print('**********************')
        exit(-1)
